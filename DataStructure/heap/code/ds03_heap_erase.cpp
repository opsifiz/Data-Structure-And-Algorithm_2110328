...
class priority_queue
{
  ...
  public:
    ...
    int find(const T& v, int idx){
      if(idx >= mSize) return -1;
      if(v > mData[idx]) return -1;
      if(mData[idx] == v) return idx;
      int lhs = find(v,2*idx+1);
      int rhs = find(v,2*idx+2);
      if(lhs != -1) return lhs;
      if(rhs != -1) return rhs;
      return -1;
    }
    ...
    void erase(const T& v) {
      ...
      mData[v_idx] = mData[mSize-1];
      mSize--;
      if(v_idx > 0 && mLess(mData[(v_idx-1)/2],mData[v_idx])){
        fixUp(v_idx);
      }else{
        fixDown(v_idx);
      }
